    ...
    var math = (function () {
        ...
        return {
            ...
            perspective_4point_transform: function (
                ...
                var Hr4 =
                    (-t42 + t41 * t9 - t55 * t2 + t46 - t48 + t55 * t11 + t51 - t21 * t9) * t30
                var Hr5 = t14
        ...
        return {
            ...
            grayscale: function (src, dst) {
                ...
                for (var i = 0; i <= srcLength_16; i += 16, j += 4) {
                    dst[j] =
                        (src[i] * coeff_r + src[i + 1] * coeff_g + src[i + 2] * coeff_b + 8192) >>
                        14
                    dst[j + 1] =
                        (src[i + 4] * coeff_r +
                            src[i + 5] * coeff_g +
...
function resize_contours(params) {
    ...
    for (var k = 0; k < params.letters.length; k++) {
        ...
        for (var n = 0; n < letter.shape.length; n++) {
            var x =
                (((letter.shape[n] % lw) + letter.x0 - params.region.x0) / params.swtscale +
                    params.xpad) *
                params.mskscale
            var y =
                ((Math.floor(letter.shape[n] / lw) + letter.y0 - params.region.y0) /
                    params.swtscale +
                    params.ypad) *
                params.mskscale
...
function erode_contours(marker) {
    ...
    for (var i = 0; i < marker.data.length; i++) {
        if (!marker.data[i]) {
            erosion.data[i + 1] =
                erosion.data[i - 1] =
                erosion.data[i + erosion.cols] =
                erosion.data[i - marker.cols] =
                    0
        }
    }
    return erosion
...
function dilate_contours(marker) {
    ...
    for (var i = 0; i < marker.data.length; i++) {
        if (marker.data[i]) {
            dilation.data[i + 1] =
                dilation.data[i - 1] =
                dilation.data[i + dilation.cols] =
                dilation.data[i - marker.cols] =
                    1
        }
    }
    return dilation
