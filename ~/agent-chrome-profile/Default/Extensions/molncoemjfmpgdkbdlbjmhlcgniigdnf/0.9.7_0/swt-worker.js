...
function partial_swt(src, params) {
    ...
    function extract_regions(direction) {
        ...
        function wrap_contours(points) {
            ...
            var median = swts.sort(function (a, b) {
                return a - b
            })[Math.floor(swts.length / 2)]
            ...
            return {
                ...
                medy:
                    y_coords.sort(function (a, b) {
                        return a - b
                    })[Math.floor(y_coords.length / 2)] - cy,
                area: area,
                contours: points,
        ...
        function wrap_lines(letters) {
            ...
            for (var i = 0; i < letters.length; i++) {
                ...
                x0 = Math.min(x0, letter.x0)
                y0 = Math.min(y0, letter.y0)
                x1 = Math.max(x1, letter.x1)
                y1 = Math.max(y1, letter.y1)
                size += letter.size
                hs += letter.height
            ...
            var dydx = slopes.sort(function (a, b) {
                return a - b
        ...
        var letters = contours.map(wrap_contours).filter(function (e) {
            if (!e) return false
        ...
        for (var i = 0; i < 2; i++) {
            // do it twice because sometimes it misses something on the first go
            lines = equivalence_classes(lines, function (r_bb, l_bb) {
                ...
                if (Math.max(r_bb.height, l_bb.height) / Math.min(r_bb.height, l_bb.height) > 1.4)
                    return false
        ...
        lines = [].concat
            .apply(
                [],
                split_lines(lines, swt).map(function (groups) {
        ...
        lines = lines
            .map(function (line) {
                ...
                var heights = line.letters.slice(1, -1).map(function (e) {
                    return e.height
                })
                var avg = mean(heights)

                // this might be a bad idea
                var heights = line.letters
                    .slice(1, -1)
                    .map(function (e) {
                        return e.height
                    })
    ...
    lines.sort(function (a, b) {
        return a.cy - b.cy
    }) // lets sort the lines top to bottom
...
function raw_swt(img_canny, img_dxdy, params) {
    ...
    for (var i = 0; i < width * height; i++) {
        ...
        while (step < max_stroke) {
            ...
            if (Math.abs(Math.abs(itheta - jtheta) - Math.PI) < Math.PI / 2) {
                // check if theta reflects the starting angle approximately
                strokes.push(i)
                var sw = Math.sqrt((jx - ix) * (jx - ix) + (jy - iy) * (jy - iy)) // derive the stroke width
                for (var k = 0; k < ray.length; k++) {
                    // iterate rays and set points along ray to minimum stroke width
    ...
    for (var k = 0; k < strokes.length; k++) {
        ...
        var median = widths.sort(function (a, b) {
            return a - b
...
function coarse_lines(letters, params) {
    ...
    for (var i = 0; i < letters.length; i++) {
        ...
        for (var j = 0; j < letters.length; j++) {
            ...
            if (
                Math.max(li.height, lj.height) / Math.min(li.height, lj.height) >
                params.height_ratio
            )
                continue
    ...
    while (pair_queue.length) {
        ...
        var merged = lca.concat(rca).sort(function (a, b) {
            return a.x1 - b.x1
        })

        for (var i = 0; i < lca.length; i++) lca[i].group = right_group
...
function find_lines(letters, params) {
    ...
    for (var i = 0; i < letters.length; i++) {
        ...
        for (var j = i + 1; j < letters.length; j++) {
            ...
            if (
                w >
                2 * Math.max(Math.min(left.height, left.width), Math.min(right.height, right.width))
            )
                continue
    ...
    function zigometer(set) {
        ...
        set.sort(function (a, b) {
            return a.x1 - b.x1
        }) // im debating whether this is a better metric than cx
        var last = set[0],
            lastdy,
            sigddy = 0
        for (var i = 1; i < set.length; i++) {
            ...
            var dy =
                Math.max(v_overlap, Math.max(last.y0, set[i].y0) - Math.min(last.y1, set[i].y1)) -
                v_overlap
    ...
    while (pair_queue.length) {
        ...
        if (lca.length > 1 || rca.length > 1) {
            ...
            var l_height = Math.max.apply(
                Math,
                lca.map(function (e) {
                    return e.height
                })
            )
            var r_height = Math.max.apply(
                Math,
                rca.map(function (e) {
                    return e.height
                })
    ...
    return regions.map(function (line) {
        ...
        for (var i = 0; i < line.letters.length - 1; i++) {
            ...
            if (next.x0 - cur.x1 > Math.sqrt(Math.min(next.area, cur.area))) {
                var streak = -1,
                    separators = 0,
                    y = Math.floor(cur.cy / 2 + next.cy / 2)
